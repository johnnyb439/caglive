      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-6">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-white hover:text-sky-blue transition-colors duration-300 whitespace-nowrap"
              >
                {link.label}
              </Link>
            ))}
            ...
            <div className="flex items-center space-x-4 ml-8">
              {/* Sign In / Create Account for non-logged in users */}
              {!user && (
                <>
                  <Link
                    href="/login"
          ...
          <div className="md:hidden flex items-center space-x-3">
            ...
            {/* American Flag for Mobile */}
            <div className="flex items-center space-x-1">
              <div className="w-6 h-4 relative bg-patriot-red">
                {/* Red and white stripes */}
                <div className="absolute inset-0 flex flex-col">
                  {[...Array(13)].map((_, i) => (
                    <div key={i} className={`flex-1 ${i % 2 === 0 ? 'bg-patriot-red' : 'bg-white'}`}></div>
                ...
                {/* Blue canton with 50 stars */}
                <div className="absolute top-0 left-0 w-[40%] h-[54%] bg-navy-blue"></div>
              </div>
