...
export default function ResumeInsightsPanel({ insights, isAnalyzing }: ResumeInsightsPanelProps) {
  ...
  if (isAnalyzing) {
    return (
      <div className="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm dark:shadow-gray-800 p-6 mb-6">
        <div className="flex items-center space-x-3 mb-4">
          <Brain className="w-6 h-6 text-blue-600" />
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white">AI Resume Insights</h3>
        </div>
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-[#1a1a1a] rounded-lg shadow-sm dark:shadow-gray-800 p-6 mb-6"
    >
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-lg font-medium text-gray-900 dark:text-white">Overall Match Score</h4>
          <div 
            className="relative"
        ...
        <div className="flex flex-wrap gap-2">
          {insights.keywordsFound.map((keyword, index) => (
            <motion.button
              ...
              className="group relative px-3 py-1.5 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 rounded-full text-sm font-medium hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors cursor-pointer"
            >
              {keyword}
      ...
      {insights.missingKeywords.length > 0 && (
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {insights.missingKeywords.map((keyword, index) => (
              <motion.button
                ...
                transition={{ delay: index * 0.05 }}
                onClick={() => copyToClipboard(keyword)}
                className="group relative px-3 py-1.5 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 rounded-full text-sm font-medium hover:bg-yellow-100 dark:hover:bg-yellow-900/30 transition-colors cursor-pointer"
              >
                {keyword}
                <span className="absolute -top-1 -right-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {copiedKeyword === keyword ? (
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400 mt-2">Click any keyword to copy to clipboard</p>
        </div>
      )}
